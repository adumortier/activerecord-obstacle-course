...
# ----------------------
  ...
  it '1. finds orders by amount' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    orders_of_500 = Order.select(:amount).where('amount = 500')
    orders_of_200 = Order.select(:amount).where('amount = 200')
    # ------------------------------------------------------------
  ...
  it '2. finds order id of smallest order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    order_id = Order.select(:id).order(:amount).first
    # @order_1 = Order.order(:amount).first
    # ------------------------------------------------------------
  ...
  it '3. finds order id of largest order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # @order_15 = Order.order('amount DESC').first
  ...
  it '5. finds multiple items by id' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    expected_objects = Item.where(id: [@item_1.id, @item_2.id, @item_4.id])
    # ------------------------------------------------------------
